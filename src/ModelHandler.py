...
class ModelHandler:
    ...
    def __init__(
        self, dataStructures: TraceMapHandler, allowed_deviation, trace_Treshold
    ) -> None:
    ...
    def __calc_time_spreads(self, trace_id) -> dict:
        ...
        for x in range(0, len(trace) - 1):
            ...
            if (
                time_spreads.get((first.get_event_name(), second.get_event_name()))
                is not None
            ):
                # relation is already in the dictionary
                time_spreads[(first.get_event_name(), second.get_event_name())].append(
                    time.total_seconds()
                )
            else:
                time_spreads[(first.get_event_name(), second.get_event_name())] = [
                    time.total_seconds()
                ]
