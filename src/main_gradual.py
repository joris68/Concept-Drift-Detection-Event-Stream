...
if __name__ == "__main__":
    ...
    try:
        with open('./ExperimentsDocker/gradual_100.csv', 'x', newline='') as file:
    ...
    for counter_1 in range(0,10):
        for trace_threshold in trace_thresholds:
                for anomaly_threshold in anomaly_thresholds:
                    ...
                    PH_length, drift_history, cohens_history = gradual_100(allowed_deviation=2.0, trace_treshold=trace_threshold, lower_boundary=220, anomaly_treshold=anomaly_threshold, cohens_boundary=13, model_epsilon=0.2)
                    execution_time = time.time() - start_time
                    try:
                        with open('./ExperimentsDocker/gradual_100.csv', 'a', newline='') as file:
                            writer = csv.writer(file)
